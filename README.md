...
即時語音轉文字與翻譯工具，使用 OpenAI Whisper 模型。
...
```bash
cd /Users/winston/Projects/whisper-live-client
uv venv
...
| 模型 | 大小 | 速度 | 準確度 | 適用場景 |
|------|------|------|--------|----------|
| `tiny` | ~75 MB | 最快 | 較低 | 快速測試 |
| `base` | ~145 MB | 快 | 基本 | 簡單對話 |
| `small` | ~488 MB | 中等 | 好 | 一般使用（推薦） |
| `medium` | ~1.5 GB | 較慢 | 很好 | 較高品質需求 |
| `large-v3` | ~3 GB | 最慢 | 最好 | 最高品質需求 |
